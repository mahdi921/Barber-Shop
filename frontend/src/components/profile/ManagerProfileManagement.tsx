...
const ManagerProfileManagement: React.FC = () => {
    const { user } = useAuth();
    const queryClient = useQueryClient();

    const managerProfile = user?.manager_profile;
    ...
    const updateMutation = useMutation({
        mutationFn: (data: any) => profileApi.updateManagerProfile(data),
        onSuccess: () => {
    ...
    return (
        <div className="space-y-6">
            {/* Telegram Setup Card */}
            <TelegramSetupCard
                hasTelegramLinked={!!user?.manager_profile?.telegram_chat_id}
                    ...
                    {isEditing ? (
                        <form onSubmit={handleSubmit} className="space-y-4">
                            <div className="text-center text-gray-500 py-4">
                                اطلاعات مدیر از طریق حساب کاربری قابل ویرایش نیست
                            </div>
                        ...
                        <div className="space-y-4">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div>
                                    <p className="text-sm font-medium text-gray-500">نوع کاربری</p>
